...
use crate::models::encrypted_vault::EncryptedVault;
...
pub fn initialize_vault_data_file(app: &tauri::AppHandle) -> Result<(), String> {
    ...
    if !vault_data_file_path.exists() {
        let initial_vault_data = EncryptedVault {
            items: Vec::new(),
...
pub fn get_vault_data_file_items(app: &tauri::AppHandle) -> Result<EncryptedVault, String> {
    let vault_data_file_path = get_vault_data_file_path(app)?;
    let vault_data = fs::read(&vault_data_file_path)
        .map_err(|_| "Failed to read vault file".to_string())?;
    let vault: EncryptedVault = serde_json::from_slice(&vault_data)
        .map_err(|_| "Failed to deserialize vault".to_string())?;

    Ok(vault)
}

pub fn write_vault_data_file_items(app: &tauri::AppHandle, new_vault: EncryptedVault) -> Result<(), String> {
    let vault_data_file_path = get_vault_data_file_path(app)?;
